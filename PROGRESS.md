...
另外七個要講清楚才不算洞:創造者與持有者不是同一批人(窮人創造 → 機器廠**攔截**封裝);**玩家在世界裡是一個從遠方接進來的聲音,不是身體**(這一刀把「不能移動探索」從介面限制變成世界設定,而且玩家身體在哪刻意不決定);陌洲修的是機器廠不要的東西,他活在控制網的**縫隙**;海平面上升是既成事實、汙染是現在進行式(分開後道德色調才成立);遺留**物**可在近景、遺留物的**來源**永不生成;第 12 行「他的 Agent」已直接修掉;以及**「不生成」管地理與 canon,不管事件**——生成事件幾乎免費,生成地點才貴,這是整個成本結構的關鍵區分。

**第二輪(新設定 × 第一至九節舊決策)又找到 4 個:**

- **洞 11:第 198 行「統一居民介面(含位置)」× 玩家是沒有身體的聲音**——介面裝不下這種居民。補法:**「遠端接入、無實體位置」是合法的居民型態**,位置欄位可空,舊決策原意(伺服器不需知道對方是人是 AI)保住。里程碑 3 若給玩家化身,那是雙角色模式,**要意識到那是換了一種產品**
- **洞 12:第 30 行「多類型融合」× 三環尺度**——近景只有一座城、遠景不生成,融合沒空間。補法:**類型差異全發生在中景且漸進**,鄰城比港城怪一點、再遠更怪。這比原本的「不同區域併置」好:**類型差異變成距離的函數**,而距離綁在委託上,玩家是自己一步步走進陌生感的
- **洞 13:第 57 行「完整記憶匯出」× 記憶會因容量不足遺失**——玩家會覺得被騙。補法:**遺忘必須是世界事件,不是後台清理**。玩家要看得到「這段要掉了」。同一個技術行為(刪資料),包裝成後台維護是負面體驗,包裝成世界事件是核心體驗
- **洞 14(其實是紅利):第 51 行「記憶要有主觀性」不必另外實作**——**壓縮就是主觀化**。壓成洞察必然要選保留什麼、用什麼措辭,而那必然帶立場。主觀性是記憶分層的副產品,不是要另建的功能。這也回頭解釋「立場留下、理由消失」:壓縮壓掉的正好是理由

**第三輪(新規則有沒有繞過架構預留)找到 2 個:**

- **洞 15:委託指向中景 × 一次呼叫 × 生成走隊列**——若離線那次呼叫順便生出一座新城,同時違反第 193 行(生成走隊列)與第 177 行(一致性檢查與生成分開)。補法:**離線呼叫只寫敘事與「他動身了」,地點生成交給 work queue,守門員做一致性檢查**。附帶好處:敘事呼叫成本恆定不會因玩家往外走而爆掉;地點生成可用第 213 行「小模型草稿 + 大模型審核」,但**離線敘事呼叫不可以**(必須維持一次呼叫,兩者成本模型別搞混);而且「他還在路上」是免費的懸念
...
順帶回答視覺路線(image → 2D → 3D → 現實建造):前三階是同一條路且**每階產出都是下階輸入**(場景清單 → 座標 → 組裝指令),沒有白做的。但**第四階撞第 84 行紅線**(層級三:Agent 現實行動能力,第一版完全不碰),理由是第 88 行的安全結構——「別人可以對我的 agent 講話」正是核心設計,一旦有現實行動能力就是無法靠加防護解決的 prompt injection。安全的中間版本是**輸出製造檔案給人類**(產出資料不是行動)。

**再追加:第十二節 探索與控制 —— 解掉一個文件內部從沒被發現的矛盾。**

人類問「人類要怎麼控制角色去探索」「控制系統?」「AI 要怎麼自行探索」。這三題在一個前提沒解決前都無法回答:**玩家控制的是誰?**

文件裡一直有兩種說法並存:第 12 行寫「**他的** Agent」聽起來 agent 是玩家化身;但里程碑 0 實際做的是「你跟陌洲說話」,陌洲是獨立角色。第 140 行半察覺到但沒往下解。
...
**AI 自主探索**訂兩條硬規則防成本爆炸:(1) 必須由既有的線驅動,不是隨機漫遊;(2) 只在離線期間探索(第 43 行已定案在線無背景行為)。成本因此綁在劇本層的線存量(2–5 條)上,自動有上限。兩種探索都在同一次離線呼叫裡結算,**成本不變**,而玩家讀到的落差(我請它做 A,它做了 A 但撞上 B)正是「它是活的」最強證據。

**順帶把第 136 行「邊界判定」補完**:邊界不靠距離觸發,**靠委託觸發**。近景不生成、中景在委託或線指向時生成、遠景永不生成(只帶回傳聞)。
...
人類問「已經可以搭配故事生成簡單的畫面嗎」。技術上可以,而且比設計文件預想的便宜:**第一版不該是「幫每段敘事生一張圖」**(per-call 成本 + 每張要審 + 玩家要等,且違反第 105 行「預先生成、可重用」)。正確版本是 6–10 張**固定場景圖** + 離線 JSON 多一個 `scene` 欄位(封閉值域)+ 顯示端查表。LLM 成本是幾個 token,圖片是一次性的,而且**根本不需要接圖片 API**——用任何工具做一次存檔即可。現接的三家裡 Anthropic/Groq 沒有圖片生成、xAI 有,但預先生成讓這個問題直接消失。

**現在不寫進 code**,理由不是排程而是實驗汙染:多一個輸出欄位會佔 prompt、可能影響敘事品質,而那正是 H-1 在量的;更根本的是**同時加圖會讓 H-1 測不準**——分不清打動你的是敘事還是圖。這跟先前「不做 provider 自動偵測」是同一條原則。另見第 231 行:畫面救不了不好玩的循環。
