...
**同時定了一條紅線:記憶額度不可用儲值虛擬幣購買。** 這個設定會讓玩家自然想到「幫它買記憶」,而那條路通向**付費才能讓 agent 記得你**——情感勒索型微交易,會直接反噬「agent 是活的」這個賣點。記憶額度只能靠世界內行動取得,與第 63 行的虛擬幣完全隔離。**必須在經濟系統實作(里程碑三)之前定死**,事後拆會很痛。
